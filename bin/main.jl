...
function run_app(host="0.0.0.0", port=PORT)
    ...
    app_style = Dict(
        "width" => "76%", "text-align" => "-webkit-center", "margin" => "2% 5%"
    )
    ...
    app.layout = html_div(; style=app_style) do
        ...
        html_div(
            "Demonstrate how to deploy a model as a dashboard application on JuliaHub."
        ),
    ...
    return run_server(app, host, port; debug=DEV_MODE)
end
